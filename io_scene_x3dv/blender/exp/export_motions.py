...
def write_animation(obj):  # pass armature object
    ...
    if root_found:
        ...
        joints=HANIM_DEF_PREFIX+"humanoid_root "+(" ".join(HANIM_DEF_PREFIX+bone.name.replace(":", "_") for bone in armature.pose.bones if bone.name != "humanoid_root"))
    else:
        ...
        joints=(" ".join(HANIM_DEF_PREFIX+bone.name.replace(":", "_") for bone in armature.pose.bones))
