...
Seg2SegJoints = {
'r_hamate|r_metacarpal_5':"r_carpometacarpal_5",
'l_carpal|l_hamate':"l_midcarpal_4_5",
'r_carpal|r_hamate':"r_midcarpal_4_5",
'l_carpal_proximal_phalanx_1|l_carpal_distal_phalanx_1':"l_carpal_interphalangeal_1",
"t1|l_clavicle":"l_sternoclavicular",
"t1|r_clavicle":"r_sternoclavicular",
"l_metatarsal_2|l_tarsal_proximal_phalanx_2":"l_metatarsophalangeal_2",
"Armature|Armature":"humanoid_root",
"Armature|sacrum":"humanoid_root",
"Armature|pelvis":"sacroiliac",
"c1|skull":"skullbase",
"c2|c1":"vc1",
"c3|c2":"vc2",
"c4|c3":"vc3",
...
def export(context, x3dv_export_settings):
    ...
    def b2xHAnimNode(obj, matrix, def_id, tag, skinCoordIndex=None, skinCoordWeight=None, motions=None, child=None):
        if matrix is not None:
            ...
            center = obj.location
        print(f"Exporting {tag} {def_id}")

        match tag:
              case     "HAnimJoint":
                  node = HAnimJoint(
                     translation=MFFloat(loc[:]),
                     scale=sca[:],
                     rotation=MFFloat(rot),
                     center=MFFloat(center[:]),
                     stiffness=MFFloat([0, 0, 0]),
                     ulimit=MFFloat([0, 0, 0]),
                     llimit=MFFloat([0, 0, 0]),
                     children=MFNode([child])
                     )
              ...
              case     "HAnimSegment":
                  geometry = Shape(
                      appearance=Appearance(material=Material(diffuseColor=SFColor(1.0))),
                          geometry=Box(size = (0.05, 0.05, 0.05))
                      )
                  child  = HAnimSite(# translation=loc[:],
                      children=MFNode([
                          Transform( 
                              translation=MFFloat(loc[:]),
                              scale=sca[:],
                              rotation=MFFloat(rot),
                              children=MFNode([geometry])
                              )
                           ])
                       )
                  node = HAnimSegment()
                  node.children=[child]
                  if def_id:
                      child.DEF=HANIM_DEF_PREFIX+def_id+"_pt"
                      child.name=def_id+"_pt"
                  if def_id:
                      node.DEF=HANIM_DEF_PREFIX+def_id
                      node.name=def_id
                  return node
    ...
    def b2xBone(bone_parent, bone, matrix, hanim_lookup, armature, skinCoordInfo):
        matrix_fallback = mathutils.Matrix()
        world = scene.world
        if export_settings['x3dv_use_hierarchy']:
            try:
                bone_matrix_world = bone.matrix
            except AttributeError:
                bone_matrix_world = matrix
            try:
                if bone_parent:
                    bone_matrix = bone_parent.matrix.inverted(matrix_fallback) @ bone_matrix_world
                else:
                    bone_matrix = bone_matrix_world
            except AttributeError:
                bone_matrix = bone_matrix_world
            bone_matrix_world_invert = bone_matrix_world.inverted(matrix_fallback)
...
#                                    skinCoordWeight.append(group.weight)
            ...
            if bone.parent:
                bone_parent_name = bone.parent.name
            else:
                bone_parent_name = armature.name

            skinCoordWeight = []
            skinCoordIndex = []
            segment = b2xHAnimNode(bone, matrix, bone.name, "HAnimSegment")
            if bone_parent_name != bone.name:
                joint_name = Seg2SegJoints[bone_parent_name+"|"+bone.name]
                try:
                    if skinCoordInfo[joint_name] is not None:
                        skinCoordIndex = skinCoordInfo[joint_name]['indices']
                        skinCoordWeight = skinCoordInfo[joint_name]['weights']
                except:
                    pass

                node = b2xHAnimNode(bone, matrix, joint_name, "HAnimJoint", skinCoordIndex=skinCoordIndex, skinCoordWeight=skinCoordWeight, child=segment)

            print(f"Info: Exporting bone {bone.name}")
            for hanim_child in hanim_lookup[bone.name]['children']:
                if hanim_child.bone:
                    if bone.name != hanim_child.name:
                        joint_name = Seg2SegJoints[bone.name+"|"+hanim_child.name]
                        child = b2xHAnimNode(hanim_child.bone, matrix, joint_name, "HAnimJoint", skinCoordIndex=skinCoordIndex, skinCoordWeight=skinCoordWeight)
                        if 'node' in locals():
                            if node.children:
                                node.children.append(child)
                            else:
                                node.children=MFNode([child])
                        else:
                            node = child
                    child = b2xBone(bone, hanim_child.bone, bone_matrix, hanim_lookup, armature, skinCoordInfo)
                    if 'node' in locals():
                        if node.children:
                            node.children.append(child)
                        else:
                    ...
                    child = b2xHAnimNode(None, matrix, hanim_child.name, "HAnimJoint", skinCoordIndex=skinCoordIndex, skinCoordWeight=skinCoordWeight)
                    if 'node' in locals():
                        if node.children:
                            node.children.append(child)
                        else:
    ...
    class HAnimNode:
        def __init__(self, name, parent_name, bone, hanim_lookup):
            print(f"Adding {name} parent {parent_name}")
            self.name = name
            self.parent_name = parent_name
            self.bone = bone
            try:
                if hanim_lookup[name] is None:
                    hanim_lookup[name] = {"bone": bone, "children" : [] }
            except KeyError:
                hanim_lookup[name] = {"bone": bone, "children" : [] }
            if parent_name:
                try:
                    hanim_lookup[parent_name]['children'].append(self)
                except KeyError:
                    hanim_lookup[parent_name] = {"bone": bone, "children" : [] }
                    hanim_lookup[parent_name]['children'].append(self)
                    ...
                    for vertex in mesh.vertices: ##in each vertex of the mesh
                        ...
                        for group in vertex.groups: #first loop to calculate the total weight and the space allowed if some are locked
                            ...
                            if group_bone in armature_bones: ##if it's a bone
                                ...
                                pass
        humanoid.skeleton = []
        hanim_lookup = {"Armature" : {"bone": None, "children" : [] }, "humanoid_root" : {"bone": None, "children" : [] }}
        HAnimNode("humanoid_root", None, None, hanim_lookup)  # populates hanim_lookup
        for bone in armature.pose.bones:
            if bone.parent:
                bone_parent_name = bone.parent.name
            else:
            ...
            if isinstance(bone.name, str):
                HAnimNode(bone.name, bone_parent_name, bone, hanim_lookup) # populates hanim_lookup
                if bone_parent_name != bone.name:
                    joint_name = Seg2SegJoints[bone_parent_name+"|"+bone.name]
                    HAnimNode(bone.name, joint_name, bone, hanim_lookup) # add another child to hanim_lookup
                    HAnimNode(joint_name, bone_parent_name, None, hanim_lookup) # populates hanim_lookup
        humanoid.skeleton = [b2xBone(obj_main, armature, obj_matrix, hanim_lookup, armature, skinCoordInfo)]

        # bones should be printed after skeleton in x3d.py. That's why I've picked out skeleton in x3d.py
